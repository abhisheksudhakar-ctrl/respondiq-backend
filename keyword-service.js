...
async function getKeywordIdeas(seedKeywords, pageUrl, locationStr, languageId) {
  ...
  // ── DIAGNOSTIC: Log raw API response structure for first result ──
  if (results.length > 0) {
    const sample = results[0];
    console.log('[RespondIQ] KW API raw sample:', JSON.stringify({
      text: sample.text,
      metrics: sample.keywordIdeaMetrics
    }).substring(0, 500));
  }
  ...
  const ideas = results.map(result => {
    ...
    // ── DIAGNOSTIC: Log raw micros for first 3 keywords to verify API values ──
    if (results.indexOf(result) < 3) {
      console.log('[RespondIQ] KW RAW:', result.text,
        '| lowMicros:', lowMicros,
        '| highMicros:', highMicros,
        '| lowDollar:', (lowMicros / 1_000_000).toFixed(4),
        '| highDollar:', (highMicros / 1_000_000).toFixed(4));
    }
    ...
    // ── CPC SANITY CAP ──
    // Test/Basic Access developer tokens can return inflated bid estimates.
    // Cap at $50 low / $150 high to prevent unrealistic values in the output.
    // These caps are conservative maximums that cover even expensive B2B keywords.
    // Remove this cap once Basic Access is approved and values are verified accurate.
    const CPC_CAP_LOW = 50;
    const CPC_CAP_HIGH = 150;
    if (lowDollars > CPC_CAP_LOW) {
      console.log('[RespondIQ] CPC CAP: "' + (result.text || '') + '" low $' + lowDollars.toFixed(2) + ' capped to $' + CPC_CAP_LOW.toFixed(2));
      lowDollars = CPC_CAP_LOW;
    }
    if (highDollars > CPC_CAP_HIGH) {
      console.log('[RespondIQ] CPC CAP: "' + (result.text || '') + '" high $' + highDollars.toFixed(2) + ' capped to $' + CPC_CAP_HIGH.toFixed(2));
      highDollars = CPC_CAP_HIGH;
    }
