...
function createRespondIqPdfBuffer(data, plan) {
  ...
  function resetPageMetrics() {
    ...
    state.bottom = doc.page.height - doc.page.margins.bottom;
    state.width = state.right - state.left;
  }
  ...
  function drawHeaderFooter() {
    ...
    doc.font('Helvetica').fontSize(7).fillColor(palette.muted)
      .text('RespondIQ - ' + campaign, state.left, doc.page.height - 28, { width: state.width * 0.72 });
    doc.text('Page ' + state.pageNo, state.right - 48, doc.page.height - 28, { width: 48, align: 'right' });
  }
  ...
  function drawClosingPage() {
    ...
    resetPageMetrics();
    const midY = 220;
    doc.font('Helvetica-Bold').fontSize(18).fillColor(palette.orange).text('RESPONDIQ', state.left, 90, { align: 'center', width: state.width });
    doc.font('Helvetica').fontSize(10).fillColor(palette.muted).text('by Responsive Media Tech Services', state.left, 115, { align: 'center', width: state.width });
    ...
    card(state.left + 88, midY + 105, state.width - 176, 86, '#fff7ed', '#ffd7c2');
    doc.font('Helvetica-Bold').fontSize(9).fillColor(palette.navy).text('NEED DEEPER STRATEGY OR FULL-SERVICE EXECUTION?', state.left + 100, midY + 125, { width: state.width - 200, align: 'center' });
    doc.font('Helvetica-Bold').fontSize(13).fillColor(palette.orange).text('hello@responsivemts.com', state.left + 100, midY + 150, { width: state.width - 200, align: 'center' });
    doc.font('Helvetica').fontSize(10).fillColor(palette.muted).text('responsivemts.com', state.left + 100, midY + 170, { width: state.width - 200, align: 'center' });
  }

  function drawCover() {
    resetPageMetrics();
    doc.rect(0, 0, doc.page.width, doc.page.height).fill('#ffffff');
    doc.font('Helvetica-Bold').fontSize(20).fillColor(palette.orange).text('RESPONDIQ', state.left, 115, { width: state.width, align: 'center' });
    doc.font('Helvetica').fontSize(10).fillColor(palette.muted).text('by Responsive Media Tech Services', state.left, 144, { width: state.width, align: 'center' });
    doc.moveTo(state.left + 180, 185).lineTo(state.right - 180, 185).lineWidth(2).strokeColor(palette.orange).stroke();
    doc.font('Helvetica-Bold').fontSize(38).fillColor(palette.ink).text('Media Plan', state.left, 230, { width: state.width, align: 'center' });
    doc.font('Helvetica').fontSize(18).fillColor(palette.ink).text(campaign, state.left + 45, 288, { width: state.width - 90, align: 'center' });
    doc.font('Helvetica').fontSize(12).fillColor(palette.muted).text('Prepared for ' + brand, state.left, 346, { width: state.width, align: 'center' });
    doc.font('Helvetica').fontSize(10).fillColor(palette.muted).text('Prepared ' + generatedAt, state.left, 372, { width: state.width, align: 'center' });
    doc.font('Helvetica').fontSize(8).fillColor(palette.muted).text('AI-generated strategy deliverable', state.left, doc.page.height - 86, { width: state.width, align: 'center' });
  }
    ...
    if (!data.competitors) {
      try {
        ...
        if (identifiedNames.length > 0) {
          competitiveIntelResults = await getCompetitiveIntelligence(identifiedNames, data.locationList && data.locationList.length ? data.locationList : data.location);
...
app.post('/api/competitive-intel', async (req, res) => {
  try {
    ...
    const results = await getCompetitiveIntelligence(capped, targetLocations);
