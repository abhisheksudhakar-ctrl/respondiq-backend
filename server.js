...
// ── Gemini Client Init ──
// Initialised lazily inside handlers so missing key gives a clean error message
...
app.get('/', (req, res) => {
  res.json({
    status: 'ok',
    service: 'RespondIQ™ Backend v12 (Gemini 3.1 Pro)',
...
// The frontend sends { role: 'developer'|'user'|'assistant', content: '...' }
// Gemini expects: system instruction + history array + final user message
// ══════════════════════════════════════════════════════════════
function convertMessagesToGemini(messages) {
  // Extract system/developer message → becomes systemInstruction
  const systemMsg = messages.find(m => m.role === 'developer' || m.role === 'system');
  const systemInstruction = systemMsg ? systemMsg.content : '';

  // Remaining messages become conversation history
  ...
  // The last message is what we send as the new turn
...
async function callGemini(systemInstruction, history, userMessage, opts = {}) {
  ...
  const model = genAI.getGenerativeModel({
    ...
    tools: [{ googleSearch: {} }],
    generationConfig: {
      // NOTE: Do NOT set temperature when using Gemini 3 models with thinking enabled.
      // Google recommends using the default temperature (1.0) for best results.
      maxOutputTokens: opts.maxOutputTokens || 8192,
      // ── MEDIUM thinking: equivalent to old Gemini 3 Pro HIGH ──
      // Balances reasoning depth, latency, and cost.
      // HIGH (Deep Think Mini) is NOT needed for media planning tasks.
      thinkingConfig: {
...
Return ONLY valid JSON — no markdown, no code fences, no explanation text.`;
    ...
    const { systemInstruction, history, lastMessage } = convertMessagesToGemini(messages);
