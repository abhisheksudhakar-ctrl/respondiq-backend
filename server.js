...
Respond with ONLY the JSON object. No explanation, no markdown.`;
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        maxOutputTokens: 512,                       // was 256 — defensive
        thinkingConfig: { thinkingBudget: 0 },      // classification, no reasoning (0 = DISABLED)
...
Respond with ONLY a JSON array of company name strings. No explanation, no markdown, no numbered lists.`;
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [{ role: 'user', parts: [{ text: prompt }] }],
      config: {
        maxOutputTokens: 1024,                     // was 512 — give headroom for 3-5 names
        thinkingConfig: { thinkingBudget: 0 },     // classification, no reasoning needed (0 = DISABLED)
        responseMimeType: 'application/json',
        responseSchema: {
          type: 'array',
          items: { type: 'string' }
    ...
    if (tokens.length >= 2) {
      const recovered = tokens.slice(0, 5);
