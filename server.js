...
app.use(cors({
  origin: function(origin, callback) {
    ...
    if (origin === ALLOWED_ORIGIN || origin === ALLOWED_ORIGIN.replace(/\/$/, '')) {
...
app.post('/api/send-email', async (req, res) => {
  ...
  if (!SERVICE_ID || !PUBLIC_KEY || !PRIVATE_KEY) {
    console.error('[RespondIQ] EmailJS env vars not configured');
...
// ── Start Server ──
app.listen(PORT, () => {
  console.log(`[RespondIQ] Backend v11 (Secure Proxy) running on port ${PORT} (Render.com)`);
  console.log(`[RespondIQ] CORS origin: ${ALLOWED_ORIGIN}`);
});
