...
const { buildBenchmarkPromptBlock, refreshBenchmarksFromWeb, getBenchmarkMeta } = require('./benchmark-data');
...
app.get('/', (req, res) => {
  res.json({
    status: 'ok',
    service: 'RespondIQ™ Backend v12.4 (Gemini 3.1 Pro + Keyword Planner + Competitive Intel + Benchmark Anchors)',
...
Return ONLY valid JSON — no markdown, no code fences, no explanation text.`;
    ...
    // Resolves industry key from the request, builds a context-aware prompt block.
    // Benchmarks are starting anchors — Gemini adjusts them based on user inputs
    // (geography, competition level, audience specificity, campaign type, etc.)
    // What they prevent: wild cross-plan inconsistency with no change in inputs.
    const industryForBenchmark = requestBody.industry || '';
...
// ── Industry-based seed keywords to supplement brand-only queries ──
// These generic terms ensure the Keyword Planner returns diverse,
// category-relevant results instead of just the brand term.
const INDUSTRY_SEED_KEYWORDS = {
  'Automotive': ['car wash near me', 'auto detailing', 'car service', 'oil change'],
  'E-commerce (DTC)': ['buy online', 'free shipping', 'best deals', 'shop now'],
  'B2B SaaS': ['software solution', 'business tools', 'enterprise platform', 'workflow automation'],
  'Local Services': ['near me', 'best local', 'affordable service', 'same day service'],
  'Healthcare': ['healthcare provider', 'medical clinic', 'telehealth', 'health services'],
  'Financial Services': ['financial advisor', 'banking services', 'investment', 'loan rates'],
  'Real Estate': ['homes for sale', 'real estate agent', 'property listing', 'buy house'],
  'Education': ['online courses', 'training program', 'certification', 'learn online'],
  'Retail': ['shop near me', 'best prices', 'store deals', 'new arrivals'],
  'Hospitality & Travel': ['hotel deals', 'book travel', 'vacation packages', 'flights'],
  'Fitness & Wellness': ['gym near me', 'personal trainer', 'fitness classes', 'wellness'],
  'Food & Beverage': ['restaurant near me', 'food delivery', 'catering service', 'menu'],
  'Technology': ['tech solutions', 'IT services', 'cloud computing', 'software'],
  'Legal': ['lawyer near me', 'legal services', 'attorney', 'law firm'],
  'Non-Profit': ['donate', 'charity', 'volunteer', 'fundraiser'],
  'Entertainment & Media': ['events near me', 'tickets', 'streaming', 'entertainment'],
  'Manufacturing': ['industrial supplier', 'custom manufacturing', 'wholesale', 'OEM'],
  'Small Business (11-50)': ['small business', 'local business', 'services near me'],
};
...
app.post('/api/keyword-ideas', async (req, res) => {
  try {
    ...
    // ── Expand seeds with industry-relevant generic terms ──
    let expandedSeeds = [...(seedKeywords || [])];
    if (industry) {
      const industrySeeds = INDUSTRY_SEED_KEYWORDS[industry] || [];
      // Also try partial match (e.g., "Automotive" matches "Automotive")
      if (industrySeeds.length === 0) {
        for (const [key, seeds] of Object.entries(INDUSTRY_SEED_KEYWORDS)) {
          if (industry.toLowerCase().includes(key.toLowerCase()) || key.toLowerCase().includes(industry.toLowerCase())) {
            industrySeeds.push(...seeds);
            break;
          }
        }
      }
      // Merge: brand seeds first, then industry seeds (deduplicated)
      const existing = new Set(expandedSeeds.map(s => s.toLowerCase()));
      for (const seed of industrySeeds) {
        if (!existing.has(seed.toLowerCase())) {
          expandedSeeds.push(seed);
          existing.add(seed.toLowerCase());
        }
      }
    }
    // Cap at 10 seeds (API limit)
    expandedSeeds = expandedSeeds.slice(0, 10);

    console.log('[RespondIQ] Keyword ideas request | seeds:', expandedSeeds.join(', ') || 'none', '| url:', pageUrl || 'none', '| location:', location || 'US', '| industry:', industry || 'none');

    const ideas = await getKeywordIdeas(expandedSeeds, pageUrl, location, languageId);

    // keyword-service.js now provides pre-formatted low_cpc/high_cpc strings
    // with the correct micros-to-dollars conversion (/ 1,000,000) + live exchange rate
...
// ── Start Server ──
app.listen(PORT, () => {
  console.log(`[RespondIQ] Backend v12.4 (Gemini 3.1 Pro | MEDIUM thinking | Benchmark Anchors) running on port ${PORT}`);
